    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          {members.length > 0 && (
            <div>
              <label className="block text-xs uppercase font-bold text-slate-400 mb-2 tracking-wider">
                담당자 <span className="text-slate-600 font-normal normal-case">(복수 선택 가능)</span>
              </label>
                  ...
                  return (
                    ...
                    >
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold text-white
                        ${selected ? 'bg-indigo-500' : 'bg-slate-600'}`}>
                        {m.name.charAt(0)}
                      </span>
                      {m.name}
                    </button>
        ...
        <div className="p-6 bg-slate-950/50 flex gap-3 border-t border-white/5">
          ...
          <button
            onClick={() => onSave({ title, description, priority, assigneeIds })}
            disabled={!title.trim()}
